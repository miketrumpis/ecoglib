...
def tdms_to_hdf5(tdms_file, h5_file, memmap=True, compression_level=0):
    ...
    for (key, val) in g_obj.properties.items():
        if type(val) == unicode:
            ...
            val = str(val)
        h5_file.create_array(h5_info, key, obj=np.array(val))
    ...
    if compression_level > 0:
        filters = tables.Filters(complevel=compression_level, complib='zlib')
    else:
        filters = None
    ...
    for n in xrange(n_col):
        ...
        d_array.append(np.asarray(ch.data)[None,:])
